...
class PhysHydra(nn.Module):
    def __init__(self, in_channels=3, out_signals=1, theta=0.5, drop_rate1=0.25, drop_rate2=0.5, frames=128):
        """
        Args:
            in_channels (int): Number of channels in the input video (e.g., 1 for grayscale, 3 for RGB, etc.).
            out_signals (int): Number of output time series signals (e.g., 1 for PPG, or more for ECG, resp, etc.).
            theta (float): Parameter for CDC_T.
            drop_rate1 (float): Dropout rate for the early blocks.
            drop_rate2 (float): Dropout rate for the later blocks.
            frames (int): Desired temporal length after pooling.
        """
        super(PhysHydra, self).__init__()

        # Update the first convolution block to use in_channels instead of a fixed 3.
        ...
        # Modify the final convolution to produce out_signals output channels.
        self.ConvBlockLast = nn.Conv3d(48, out_signals, [1, 1, 1], stride=1, padding=0)
        ...
        # Pool to the desired temporal length (frames) and collapse spatial dimensions.
    ...
    def forward(self, x):
        # x shape: [batch, channels, length, width, height]
        ...
        s_x1 = self.fuse_1(s_x1, f_x1)  # Lateral connection
        ...
        s_x2 = self.fuse_2(s_x2, f_x2)  # Lateral connection
        ...
        # Pool spatial dimensions and adjust temporal length
        x_final = self.poolspa(x_final)
        x_final = self.ConvBlockLast(x_final)  # Now has shape [batch, out_signals, frames, 1, 1]

        # Squeeze the spatial dimensions to get output shape [batch, out_signals, frames]
        signals = x_final.squeeze(-1).squeeze(-1)
        return signals