"""pure-preprocess: align + resize PURE recordings into zarr stores.

Built from the template cacher; dataset-agnostic apart from the two constants. The flow per
...
import pure_cacher
from pure_cacher.scan import RecordingInfo, discover_recordings, scan_recording
from pure_cacher.traces import (
    TRACE_UNITS, AlignedStream, align_stream, read_stream_traces,
)
from pure_cacher.video import decode_video
from pure_cacher.writer import (
    NOMINAL_FPS,
    covers,
    init_store,
    mark_complete,
...
#: PURE is one RGB camera: one perspective, one modality.
        ...
        root = init_store(
            store_path,
            {
                **rec.attrs,
                ...
                "tool_version": pure_cacher.__version__,
            },
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="pure-preprocess",
        description="Align (and optionally resize) PURE recordings into zarr stores.",
    )
    parser.add_argument("--input-dir", type=Path, required=True,
                        help="Directory holding the SS-TT recording directories")
    ...
    parser.add_argument("--version", action="version", version=pure_cacher.__version__)
    return parser
